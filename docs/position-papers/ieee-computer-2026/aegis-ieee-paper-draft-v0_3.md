...
**Kenneth Tannenbaum**
AEGIS Initiative, PO Box 235, Lovettsville, Virginia 20180-0235 USA
...
Autonomous AI agents are executing consequential actions at scale against operational infrastructure—calling APIs, modifying files, and managing financial transactions. Current governance approaches focus on behavioral alignment: shaping what agents say. This paper addresses a structurally different problem: governing what agents *do* at the infrastructure boundary. We present AEGIS, a constitutional governance architecture enforcing deterministic policy at the agent action boundary—post-reasoning, pre-execution—making unauthorized actions structurally unavailable rather than merely discouraged. AEGIS combines four properties absent from existing frameworks: constitutional enforcement at the action boundary; federated multi-institution trust with cryptographic identity; tamper-evident audit trails; and alignment with the NIST AI Risk Management Framework across all four governance functions. We map the architecture to Anderson's reference monitor model (1972) and Saltzer and Schroeder's design principles (1975), and differentiate AEGIS from representative related work. Real-world agentic system failures documented in 2026 demonstrate the governance gap AEGIS addresses.
...
The 2025 AI Agent Index documented the scale of this risk: production deployments of autonomous agents grew by an order of magnitude within a twelve-month period, with deployment in financial services, healthcare, software development, and security operations.¹¹ This growth has materially outpaced the development of governance infrastructure suited to the action layer.
...
The *Governance Gateway* is the single entry point for all AGP-1 requests. It validates request schemas, authenticates actor identity via JWT or mutual TLS, applies rate limiting, and routes to the Decision Engine. This design reflects zero trust principles: every request is authenticated and authorized regardless of origin.²⁰ All gateway errors result in DENY responses. There is no error path that returns an implicit ALLOW or passes the request through to infrastructure without evaluation.
...
*Federation publisher trust* governs how much weight to assign to governance signals received from remote nodes. It is computed as a weighted composite of five factors: baseline authority class (B), historical signal accuracy (H), consistency and quality (Q), audit posture (A), and federation reputation (F), per the normative formula T = 0.30B + 0.25H + 0.20Q + 0.15A + 0.10F (GFN-1 §3.7). Authority classes range from L0\_SYSTEM, operated by AEGIS security research, to L3\_CONTRIBUTOR for community nodes. Publisher scores decay during inactivity (T\_decayed = T × e^(−λt), λ=0.01, half-life ≈69 days, GFN-1 §3.8), encoding a straightforward operational principle: evidence staleness is a legitimate input to trust evaluation. This is consistent with how temporal factors are treated in analogous trust-sensitive systems.⁹ The decay function takes time as an explicit, logged input parameter; identical inputs always yield identical output on any node, preserving local determinism. When a publisher's authority class disposition and computed score disposition conflict, the more restrictive applies (GFN-1 §10.1.1).
...
The AEGIS specification has undergone external peer review from Mattijs Moens (Founder, Sovereign Shield). Moens identified a structural flaw in the prior RFC-0004 trust model: the original composite score collapsed security signals and operational reputation into a single number, creating a condition where accumulated reputation could implicitly soften a security gate. His framing identified this as a category error — one that cannot be resolved by parameter tuning — and proposed the two-layer separation model that is now normative in RFC-0004 v0.4 §5. The specification work is the AEGIS Initiative's; the foundational insight is Moens's. He also raised a concern regarding the determinism properties of the time-dependent decay mechanic in GFN-1 §3.8. The AEGIS position is that determinism is local and peer-to-peer: time is an explicit, logged input to the decay function, not a stochastic element, and identical inputs always yield identical output on any node — consistent with how temporal factors are treated in TLS certificate expiry, Kerberos ticket lifetimes, and AML systems. This objection is considered resolved; the decay mechanic and the trust layer separation are both documented in the public repository (github.com/finnoybu/aegis-governance/discussions/72).

The AEGIS architecture has also received independent external validation from a convergent implementation. Nathan Freestone (Founder, The Elora Taurus Project) independently developed a governance runtime for small-to-medium business AI deployments, approaching the problem from the infrastructure layer outward rather than from the agent layer inward. Both systems independently concluded that inference output cannot carry its own authorization, that governance cannot share a compute boundary with what it governs, and that audit trails must be append-only, hash-linked, and forensically replayable. Three architectural patterns from the Elora implementation — append-only pipeline provenance, stateless execution units, and cryptographic agent registration as a Doctrine-layer enforcement mechanism — have been incorporated into the AEGIS specification with Freestone's explicit permission. The independent convergence of two systems on the same execution boundary from opposite architectural directions is evidence that the boundary is structurally load-bearing in the problem space, not a design preference of either implementation.
...
Two normative definitions are deferred as open problems in the current specification. First, the minimum authority claim set required for a commit decision to be considered admissible is not yet defined. The AEGIS Authority Binding sub-spec specifies that authority is captured at proposal time and re-validated at commit, but does not enumerate the minimum fields that constitute a complete admissibility proof. This is an active research question; the Elora Taurus project has independently identified the same gap in its own specification. Second, the replay completeness threshold required for production certification is not normatively specified. AEGIS audit trails are tamper-evident through hash-chaining and append-only enforcement, but the fraction of governance decisions that must be fully reproducible from stored evidence before a deployment is considered production-certified has not been stated. Both of these definitions will be addressed in forthcoming RFC-0010 and RFC-0011 revisions.
...
[9] A. Josang, R. Ismail, and C. Boyd, "A survey of trust and reputation systems for online service provision," Decision Support Systems, vol. 43, no. 2, pp. 618–644, Mar. 2007.

[10] S. Rodriguez Garzon et al., "LOKA: A Framework for AI Agent Identity Using DIDs and Verifiable Credentials," arXiv:2511.02841, 2025.

[11] R. Chan et al., "The 2025 AI Agent Index," arXiv:2602.17753, 2025.

[12] G. Syros et al., "SAGA: Security Architecture for Governing AI Agentic Systems," in Proc. NDSS, 2026.

[13] K. Arunachalam, A. Kayyidavazhiyil, and P. Santikellur, "POLYNIX: A Hybrid Policy Enforcement Framework for Zero-Trust Security in Virtualized Systems," in Proc. IEEE CCNC, 2026, doi: 10.1109/CCNC65079.2026.11366307.

[14] A. Baird, A. Panda, H. Pearce, S. Pinisetty, and P. Roop, "Scalable Security Enforcement for Cyber Physical Systems," IEEE Access, vol. 12, pp. 14385–14410, 2024, doi: 10.1109/ACCESS.2024.3357714.

[15] H. Pearce, S. Pinisetty, P. S. Roop, M. M. Y. Kuo, and A. Ukil, "Smart I/O Modules for Mitigating Cyber-Physical Attacks on Industrial Control Systems," IEEE Trans. Ind. Informat., vol. 16, no. 7, pp. 4659–4669, Jul. 2020, doi: 10.1109/TII.2019.2945520.

[16] S. Majumdar et al., "ProSAS: Proactive Security Auditing System for Clouds," IEEE Trans. Dependable Secure Comput., vol. 19, no. 4, pp. 2517–2534, Jul./Aug. 2022, doi: 10.1109/TDSC.2021.3062204.

[17] P. F. Christiano, J. Leike, T. Brown, M. Martic, S. Legg, and D. Amodei, "Deep reinforcement learning from human preferences," in Proc. NeurIPS, vol. 30, 2017.

[18] Y. Bai et al., "Constitutional AI: Harmlessness from AI feedback," arXiv:2212.08073, 2022.

[19] F. B. Schneider, "Enforceable security policies," ACM Trans. Inf. Syst. Secur., vol. 3, no. 1, pp. 30–50, Feb. 2000.

[20] S. Rose, O. Borchert, S. Mitchell, and S. Connelly, "Zero trust architecture," NIST SP 800-207, Aug. 2020.
...
*[END OF DRAFT v0.3 — 2026-03-24]*

*Draft notes for revision:*
*- [x] Trust model section updated — two-layer model per RFC-0004 v0.4 (federation publisher trust + agent runtime trust)*
*- [x] Peer review section updated — correct Mattijs credit (category error finding, not doc fix); Nathan Freestone/Elora convergence added*
*- [x] Acknowledgments updated — Mattijs and Nathan both credited correctly*
*- [x] Limitations expanded — replay completeness SLO and minimum authority claim set deferred explicitly (RFC-0010/0011)*
*- [x] "immutable tamper-evident" → "tamper-evident" in abstract per RFC-0010 gap-004*
*- [x] RFC-0004 v0.4 blocker cleared*
*- [ ] Author bio: add ORCID before submission*
...
*- [ ] Normative source audit (RFC-0009 prerequisite) — target before 2026-03-22 IEEE outline*
*- [ ] Nathan's Minimum Governance Contract Schema — incorporate when received*
