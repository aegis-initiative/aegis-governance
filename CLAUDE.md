...
| ATX-1 site/public deploy script | `sites/governance/scripts/sync-atx.mjs` (runs in `predev`/`prebuild`; `--check` for CI drift detection) |

`sites/governance/public/atx-1/*` and `sites/governance/public/schemas/atx-technique.schema.json` are BUILD OUTPUTS — do not edit by hand. Bump `atx-meta.json#version` and re-run `npm run sync-atx` instead.
