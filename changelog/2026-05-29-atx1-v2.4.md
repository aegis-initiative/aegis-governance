...
> **Reviewer note:** `AGP Progress-Signal Verification` is a newly-named mechanism in this entry. Confirm against the canonical AGP-1 mechanism registry, or rename to an existing mechanism, before tagging.
...
- `site/public/atx-1/*` (regenerated build outputs via `npm run sync-atx`: techniques.json, version-mapping.json, index.json, VERSION)
- `changelog/2026-05-29-atx1-v2.4.md` (this file) + `changelog/INDEX.md`

### Pending follow-ups (before tag / deposit)

- **Regenerate derived artifacts** to include T6003.*: STIX 2.1 bundle (`stix/atx-1-bundle.json`), ATT&CK Navigator layer, ATM-1 mapping, regulatory cross-reference. CI does not gate on these; they currently reflect 58 techniques.
- **Human-readable taxonomy doc** (`docs/atx/v2/ATX-1_TECHNIQUE_TAXONOMY.md`) — add the T6003 section.
- Confirm `AGP Progress-Signal Verification` against the AGP-1 mechanism registry (see reviewer note above).
